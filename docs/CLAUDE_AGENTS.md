...
- **Analyzes**: NIFTY 50/NIFTY Bank indices, macro indicators, global correlations
...
- **File**: `prism-in/in_stock_tracking_agent.py`
...
### ~~User Consultation Team (2 Agents)~~ — Archived

> Note: The Telegram AI bot (Portfolio Consultant & Dialogue Manager) was part of the KR module and has been archived to `_archive/prism-kr/`. These agents may be re-implemented for the India module in the future.
...
```python
# Pattern in prism-in/cores/in_analysis.py
async def analyze_stock(company_name, ticker, reference_date, language="en"):
    # 1. Get agent directory
    agents = get_in_agent_directory(company_name, ticker, reference_date,
...
def create_your_agent(company_name, ticker, reference_date, language="en"):
    """
    ...
    return Agent(
        ...
        mcp_servers=["yahoo_finance", "perplexity"],
    )

# Register in prism-in/cores/agents/
