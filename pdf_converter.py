...
def _extract_report_info(md_content: str) -> dict:
    ...
    # Method 1: Extract from H1 title like "# Reliance Industries (RELIANCE) Analysis Report"
    ...
    # Method 2: Extract from content pattern like "As of YYYY-MM-DD, CompanyName" or "CompanyName(TICKER)"
    if not info["company_name"]:
        # English pattern: "As of YYYY-MM-DD, CompanyName" or "CompanyName(TICKER)"
        en_pattern = re.search(r'(?:As of|Based on)\s+\d{4}[-./]\d{2}[-./]\d{2}[,\s]+([A-Za-z\s]+?)(?:\s+is|\s+has|\()', md_content, re.IGNORECASE)
        if en_pattern:
            info["company_name"] = en_pattern.group(1).strip()

    # Method 3: Extract stock code from content like "CompanyName(TICKER)" or "(RELIANCE)"
    if not info["company_code"]:
        # NSE ticker (uppercase letters, may include numbers)
        ticker_match = re.search(r'[\(\（]([A-Z][A-Z0-9]{1,15})[\)\）]', md_content)
        if ticker_match:
            info["company_code"] = ticker_match.group(1)

    # Extract date from content like "As of 2025-01-24" or "Publication Date: 2025-01-24"
    date_patterns = [
        r'(?:As of|Based on)\s+(\d{4}[-./]\d{2}[-./]\d{2})',
        r'(?:Publication Date|Report Date|Generated)[:\s]*(\d{4}[-./]\d{2}[-./]\d{2})',
        r'(\d{4}[-./]\d{2}[-./]\d{2})\s*(?:as of|current)',
    ]
...
def markdown_to_html(md_file_path, add_css=True, add_theme=False, logo_path=None, enable_watermark=False, watermark_opacity=0.02):
    ...
    try:
        ...
        # Report type is always English for India market
        report_type = "Analysis Report"
...
def pdf_to_markdown_text(pdf_path):
    # If it's already a markdown file, read it directly
