...
class TelegramSummaryGenerator:
    ...
    async def generate_telegram_message(self, report_content, metadata, trigger_type, from_lang="ko", to_lang="ko"):
        ...
        prompt_message = f"""The following is a detailed analysis report for {metadata['stock_name']}({metadata['stock_code']}).
            This stock was detected by the {trigger_type} trigger.
        ...
        if metadata.get('trigger_mode') == 'morning':
            ...
            prompt_message += "\n⚠️ Note: This data is from 10 minutes after market open. Current conditions may differ."
        ...
        if isinstance(response, str):
            ...
            content_match = re.search(r'(📊|📈|📉|💰|⚠️|🔍)', cleaned_response)
            message_end = re.search(r'This information is for reference only\. Investment decisions and responsibilities lie with the investor\.', cleaned_response)

            if content_match and message_end:
                return cleaned_response[content_match.start():message_end.end()]
        ...
        content_match = re.search(r'(📊|📈|📉|💰|⚠️|🔍).*?This information is for reference only\. Investment decisions and responsibilities lie with the investor\.', response_str, re.DOTALL)
    ...
    1. Current Price: (unavailable)
    2. Recent Trend: (unavailable)
    3. Key Checkpoints: See detailed analysis report.

    ⚠️ Unable to display details due to auto-generation error. Please check the full report.
    This information is for reference only. Investment decisions and responsibilities lie with the investor."""
